...
| 3 | Calendar | Google Calendar — 15 distinct calendars. Each calendar's profile, purpose and write owners are set in `~/.config/achios/calendars.json`; Job and GCash have no write owner, so no agent writes them. | connected. `scripts/gcal.py` is the only Calendar client; the briefs, health check and cohesion call it. Calendars, write owners and the schedule set live in the private `~/.config/achios/calendars.json` (shape in `config/calendars.example.json`). | OAuth ×4 in Production mode on achibuntu (`~/.config/gws-*`) | 2026-09-17 |
...
| 6 | Project / task tracking | Canvas LMS + achiOS task, Calendar and semantic preference cohesion | `scripts/cohesion.py` provides versioned task and Calendar operations plus sourced semantic preferences in SQLite. `scripts/semantic_review.py` reviews pending evidence daily through tool-free Gemini inference. achiCore #148 owns Telegram source capture and exact-quote validation. | private Canvas cookie jar under ~/.config/achios/canvas; Google OAuth profiles under ~/.config/gws-*; `GEMINI_API_KEY` under ~/.config/achios; cohesion and learning state under ~/.local/state/achios | 2026-09-22 |
...
## Asta release under review

The Asta branch adds private SQLite nutrition records, cached USDA food search,
