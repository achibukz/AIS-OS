...
| 8 | Notifications | Telegram — achiOS bot (its own bot, not Hermes') | `script` — `scripts/telegram_notify.py` is the shared sender; systemd user timers on achibuntu (`OnCalendar=… Asia/Manila`, `Persistent=true`). Not cron — it ignores `CRON_TZ`. Wire new jobs with the `cron-telegram` skill. | bot token + chat id in `~/.config/achios/telegram.env`, mode 600 | 2026-08-17 |
