...
| 6 | Project / task tracking | Canvas LMS (school tasks) + achiMem (general, proposed) + achiOS task and Calendar cohesion | Canvas client/cache merged in PR #33. `scripts/cohesion.py` provides versioned task and Calendar submit, context and capabilities operations backed by SQLite. Its Google adapter goes through `scripts/gcal.py` and names calendars from the calendar config. Telegram integration remains achiCore #148. | private Ubuntu cookie jar under ~/.config/achios/canvas; Google OAuth profiles under ~/.config/gws-*; cohesion state under ~/.local/state/achios | 2026-09-11 |
