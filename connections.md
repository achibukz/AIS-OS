...
| 3 | Calendar | Google Calendar — 15 distinct calendars. Default to `gws-personal` (school, thesis, Personal, Bdayy, Canvas feed); `gws-dlsu` for DLSU event detail; `gws-work` to write to Job. | connected. `scripts/gcal.py` is the only Calendar client; the briefs, health check and cohesion call it. Calendars, write owners and the schedule set live in the private `~/.config/achios/calendars.json` (shape in `config/calendars.example.json`). | OAuth ×4 in Production mode on achibuntu (`~/.config/gws-*`) | 2026-09-17 |
