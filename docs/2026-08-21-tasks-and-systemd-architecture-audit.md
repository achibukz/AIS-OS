...
* **Location:** `achiAgy/src/bot.py:968-975`
* **Problem:** 
  ```python
...
* **Locations:**
  - `AIS-OS/scripts/daily_brief.py:40-108`
  - `AIS-OS/scripts/tasks_digest.py:34-109`
  - `AIS-OS/scripts/evening_debrief.py`
* **Problem:** Each script duplicates `TASK_RE`, `DUE_RE`, `PRIORITY_RE`, `AREA_RE`, `FENCE_RE`, `LINK_RE`, `strip_markup`, and `parse_active_tasks`.
* **Discrepancy Example:** `tasks_digest.py` includes `MD_LINK_RE` to strip markdown links (`[Label](url)` syntax), but `daily_brief.py` is missing it, causing markdown syntax to leak into the morning brief.
...
* **Location:** `achiAgy/src/bot.py:975, 986, 995`
...
* **Location:** `AIS-OS/scripts/vault_inbox_sync.py`
...
* **Location:** `AIS-OS/scripts/service_failure_alert.py`
